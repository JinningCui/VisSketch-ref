...
def prepare_winner_id_prompt(example):
    fen = example["fen"]
    text_query = (
        "You are given a chess game position in FEN.\n\n"
        "YOUR TASK is to predict the final winner of the game: 'white', 'black', or 'draw'.\n"
    ...
    text_query += (
        ...
        "Never answer 'ongoing'. You must output exactly one of 'white', 'black', or 'draw'.\n\n"
    )
    text_query += f"Current FEN:\n{fen}\n\n"
    text_query += "Respond with exactly one of: 'white', 'black', or 'draw'."
    return text_query
...
TASK2PROMPT = {
    ...
    "winner_id": prepare_winner_id_prompt
}
